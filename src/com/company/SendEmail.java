...
public class SendEmail {
    public static void sendNewEmail(String emailDetails) throws IOException {
        String[] data=emailDetails.split(",");
        sendEmailHandler(data[0],data[1],data[2]);

    }

    public static void sendEmailHandler(String email, String subject,String content) throws IOException {

        SimpleDateFormat formatter = new SimpleDateFormat("yyyy/MM/dd");
        Date date = new Date();
        String sentDate = formatter.format(date);

        Email sentEmail = new Email(email, subject, sentDate);
        Serialize.serialize();
